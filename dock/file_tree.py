...
from .utils import load_config, resized_icon
import os
...
class FileTree(tk.Frame):
    def __init__(self, root, code_area, **kwargs):
        ...
        self.tree.bind("<<TreeviewSelect>>", self.open_item, add='+')
    ...
    def update_folder_icon(self, is_open: bool):
        ...
        if is_open:
            self.tree.item(item_id, image=self.FOLDER_OPEN_ICON)
        else:
            self.tree.item(item_id, image=self.FOLDER_CLOSED_ICON)
    ...
    def rebuild_recursive(self, directory, parent):
        for item in os.listdir(directory):
            ...
            item_id = self.tree.insert(parent, "end", text=item, values=(item_path))
    ...
    def open_item(self, event):
        ...
        if item:
            ...
            item_path = self.tree.item(item)["values"][0]  # Retrieve the full path from the tags
